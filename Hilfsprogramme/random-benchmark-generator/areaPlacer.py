...
class AreaPlacer:
    ...
    def place_areas(self):
        while self.currentArea <= self.X * self.Y:
            ...
            while placed_pixels < random_pixel:
                possible_positions = []
                #print("placed_pixels " + str(placed_pixels))
                for point_x, point_y in placed_points:
                    possible_positions += self.possible_places(point_x, point_y)
                possible_positions = list(set(possible_positions))
                possible_positions = self.possible_places(x, y)
                #print("possible_positions " + str(possible_positions))
                if not possible_positions:
                    #print("no possible positions")
                    break

                x, y = random.choice(possible_positions)
                #print("(x, y) " + str(x) + ", " + str(y) + " = " + str(self.currentArea))
                self.grid[y][x] = self.currentArea
                placed_pixels += 1
    ...
    def save_benchmark(self, width, height, shapes, num_files, text, maxOneHash):
        ...
        if not os.path.exists(f"benchmarks/{text}"):
            os.mkdir(f"benchmarks/{text}")

        benchmark = Benchmark(width, height, shapes)
        with open(f"benchmarks/{text}/benchmark_{width}_{height}-{text}-mOH={maxOneHash}-{num_files}.txt", "w") as file:
